...
import highway_env  # noqa: F401 — triggers env registration
from highway_env.envs.intersection_pz_env import (
...
def test_obs_in_observation_space(intersection_env):
    ...
    while intersection_env.agents:
        actions = {a: intersection_env.action_space(a).sample() for a in intersection_env.agents}
        obs, rewards, terminations, truncations, infos = intersection_env.step(actions)
        for agent, agent_obs in obs.items():
            assert intersection_env.observation_space(agent).contains(agent_obs), (
    ...
    while intersection_env.agents:
        ...
        for agent in pre_step_agents:
            if terminations.get(agent, False) or truncations.get(agent, False):
                assert agent not in intersection_env.agents, (
