...
It vastly reduces the generation process time and is also used in places such as LaneLidarObservation and PartitionedRoadNetwork.

A set of functions that support grid partitioning is available under `generation/spatial_hash.py`


## RandomRoadEnv Description
...
- Varying car sizes
- Introducing ill-behaved agents to complicate the problem of safety
	- Choose one car to be completely blind to all other cars. It will not know when to yield, so other cars will be forced to
    - Reintroduce an early, less competent version of the agent amongst the more advanced agents
	- Introduce 'evil agents' whose sole goal is to find and crash into another agent

## Todo
- Extend RandomRoadEnv to use multiple agents
- Allow LaneLidarObservation to observe non-Polylanes
- Allow LaneLidarObservation to use a regular RoadNetwork instead of just PartitionedRoadNetwork
- Allow the rays in LaneLidarObservation to be more concentrated at the head for higher forward resolution
