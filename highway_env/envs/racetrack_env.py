...
class RacetrackEnv(AbstractEnv):
    ...
    def _reward(self, action: np.ndarray) -> float:
        ...
        reward = self.config["collision_reward"] * self.vehicle.crashed \
            + 1/(1+4*lateral**2)\
            -0.3*np.linalg.norm(action)
        reward = reward if self.vehicle.on_road else self.config["collision_reward"]
        return utils.lmap(reward, [self.config["collision_reward"], self.config["high_speed_reward"]], [0, 1])
    ...
    def _make_vehicles(self) -> None:
        ...
        ego_vehicle = self.action_type.vehicle_class.make_on_lane(
            self.road,
            ("a", "b", self.np_random.randint(1)),
            longitudinal=self.np_random.uniform(low=0, high=30),
            speed=None
        )
        ...
        # Other vehicles
        for i in range(self.np_random.randint(1, 2)):
